...
<h1 class="text-2xl font-bold text-gray-800 mb-4">Create Data Rooms</h1>
    <form action="{{ route('rooms.store') }}" method="POST" class="w-[90%]" enctype="multipart/form-data">
        @csrf
      <!-- Input Nama -->
      <div class="mb-4">
        <label for="room" class="block mb-1 text-gray-700 font-medium">Name Room</label>
        <input type="text" id="room" name="room" class="w-full outline-none transition duration-300 outline-[1px] mt-2 p-2 border-gray-300 rounded-md shadow-sm focus:outline-blue-500 focus:ouline-blue-500" placeholder="Room name..." value="{{ old('room') }}"/>
        @error('room')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror

        <label for="price" class="mt-5 mb-1 block text-gray-700 font-medium">Price</label>
        <input type="number" id="price" name="price" class="w-full outline-none transition duration-300 outline-[1px] mt-2 p-2 border-gray-300 rounded-md shadow-sm focus:outline-blue-500 focus:ouline-blue-500" placeholder="Price..." value="{{ old('price') }}"/>
        @error('price')
          <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
        ...
        <!-- Preview Image -->
        <div class="mt-4">
            <p class="text-sm text-gray-500" id="previewText">No image selected.</p>
            <img id="previewImage" src="" alt="Image Preview"
                class="hidden w-full max-h-64 object-cover rounded-lg border border-gray-200">
        </div>
      </div>
